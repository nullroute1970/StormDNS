...
📢 **کانال تلگرام / Telegram Channel:** [https://t.me/nulllroute1970](https://t.me/nulllroute1970)

پروژه **StormDNS** یک پروژهٔ علمی-تحقیقاتی برای انتقال داده‌های TCP از طریق درخواست‌ها و پاسخ‌های DNS است. این پروژه در هدف کلی شبیه پروژه‌هایی مانند DNSTT یا SlipStream است، اما از نظر ساختار و روش پیاده‌سازی تفاوت‌های بنیادین دارد و رویکرد متفاوتی را دنبال می‌کند.
...
### 📊  مقایسه StormDNS با پروژه‌های مشابه:

| ویژگی | SlipStream | DNSTT | StormDNS |
| :--- | :--- | :--- | :--- |
...
پروژه StormDNS صرفاً یک ایدهٔ علمی و آموزشی است و بر همین اساس طراحی و پیاده‌سازی شده است.
...
برای دریافت آخرین اخبار، نسخه‌ها و اطلاعیه‌های پروژه، کانال تلگرام ما را دنبال کنید: [Telegram Channel](https://t.me/nulllroute1970)
...
نمای کلی و مختصر از قابلیت‌های اصلی StormDNS:
...
##### آرگومان‌های خط فرمان اسکریپت

اسکریپت نصب از آرگومان‌های زیر پشتیبانی می‌کند که می‌توانید آن‌ها را مستقیماً با همان دستور `bash <(curl ...)` به کار ببرید:

| گزینه | توضیح |
| --- | --- |
| `-v <VERSION>`, `--version <VERSION>` | نصب نسخهٔ مشخصی از StormDNS با تگ موردنظر (مثلاً `v1.2.3`) به جای آخرین نسخه. |
| `-u`, `--uninstall` | حذف کامل StormDNS: متوقف کردن و پاک کردن سرویس `stormdns` در systemd، حذف تنظیمات کرنل/محدودیت‌ها و پاک‌سازی فایل اجرایی، `server_config.toml` و `encrypt_key.txt` در دایرکتوری جاری. |
...
# حذف StormDNS (دستور را در همان دایرکتوری‌ای اجرا کنید که StormDNS را نصب کرده‌اید):
bash <(curl -Ls https://raw.githubusercontent.com/nullroute1970/StormDNS/main/server_linux_install.sh) --uninstall
```

> توجه: برای حذف، دستور را در همان دایرکتوری اجرای نصب اولیه اجرا کنید تا اسکریپت بتواند فایل اجرایی، فایل پیکربندی و کلید رمزنگاری را پیدا و پاک کند.
...
| :--- | :--- | :--- | :--- |
| ویندوز (Windows) 🪟 | `AMD64` (64-bit) | ویندوز ۱۰ و ۱۱ | [دانلود نسخه ویندوز ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Windows_AMD64.zip) |
| ویندوز (Windows) 🪟 | `x86` (32-bit) | سیستم‌های قدیمی ۳۲ بیتی ویندوز | [دانلود نسخه ویندوز x86 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Windows_X86.zip) |
| ویندوز (Windows) 🪟 | `ARM64` | دستگاه‌های ویندوزی مبتنی بر ARM | [دانلود نسخه ویندوز ARM64 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Windows_ARM64.zip) |
| مک‌اواس (macOS) 🍎 | `ARM64` | مک‌های جدید (سری M1 / M2 / M3) | [دانلود نسخه مک (Apple Silicon) ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_MacOS_ARM64.zip) |
| مک‌اواس (macOS) 🍎 | `AMD64` | مک‌های اینتل | [دانلود نسخه مک اینتل ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_MacOS_AMD64.zip) |
| لینوکس (Linux) 🐧 | `AMD64` (64-bit) | توزیع‌های جدید (اوبونتو ۲۲.۰۴+، دبیان ۱۲+) | [دانلود نسخه لینوکس (جدید) ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux_AMD64.zip) |
| لینوکس (Linux) 🐧 | `x86` (32-bit) | سیستم‌های قدیمی ۳۲ بیتی لینوکس | [دانلود نسخه لینوکس x86 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux_X86.zip) |
| لینوکس (Legacy) 🐧 | `AMD64` (64-bit) | توزیع‌های قدیمی (اوبونتو ۲۰.۰۴، دبیان ۱۱) | [دانلود نسخه لینوکس (سازگاری بالا) ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux-Legacy_AMD64.zip) |
| لینوکس (Legacy) 🐧 | `ARM64` | سیستم‌های ARM64 قدیمی‌تر که سازگاری بیشتری می‌خواهند | [دانلود نسخه لینوکس Legacy ARM64 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux-Legacy_ARM64.zip) |
| لینوکس (ARM) 🐧 | `ARM64` | سرورهای ARM، رزبری‌پای و بردهای مشابه | [دانلود نسخه لینوکس (ARM) ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux_ARM64.zip) |
| لینوکس (ARM) 🐧 | `ARMv7` | بردهای ARM ۳۲ بیتی و دستگاه‌های قدیمی‌تر | [دانلود نسخه لینوکس ARMv7 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux_ARMV7.zip) |
| لینوکس (ARM) 🐧 | `ARMv6` | بردهای ARM قدیمی‌تر و سیستم‌های سبک لینوکسی | [دانلود نسخه لینوکس ARMv6 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux_ARMV6.zip) |
| لینوکس (ARM) 🐧 | `ARMv5` | دستگاه‌های ARM خیلی قدیمی و سیستم‌های embedded | [دانلود نسخه لینوکس ARMv5 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux_ARMV5.zip) |
| لینوکس (Linux) 🐧 | `RISCV64` | بردها و سرورهای لینوکسی مبتنی بر RISC-V | [دانلود نسخه لینوکس RISCV64 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux_RISCV64.zip) |
| لینوکس (MIPS) 🐧 | `MIPS` | سیستم‌ها و روترهای لینوکسی MIPS با endian بزرگ | [دانلود نسخه لینوکس MIPS ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux_MIPS.zip) |
| لینوکس (MIPS) 🐧 | `MIPSLE` | سیستم‌ها و روترهای لینوکسی MIPS با endian کوچک | [دانلود نسخه لینوکس MIPSLE ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux_MIPSLE.zip) |
| لینوکس (MIPS) 🐧 | `MIPS64` | سیستم‌های ۶۴ بیتی MIPS با endian بزرگ | [دانلود نسخه لینوکس MIPS64 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux_MIPS64.zip) |
| لینوکس (MIPS) 🐧 | `MIPS64LE` | سیستم‌های ۶۴ بیتی MIPS با endian کوچک | [دانلود نسخه لینوکس MIPS64LE ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Linux_MIPS64LE.zip) |
| ترموکس / اندروید 📱 | `ARM64` | گوشی‌های اندرویدی جدید با Termux | [دانلود نسخه ترموکس ARM64 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Termux_ARM64.zip) |
| ترموکس / اندروید 📱 | `ARMv7` | گوشی‌های قدیمی‌تر با محیط ۳۲ بیتی Termux | [دانلود نسخه ترموکس ARMv7 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Client_Termux_ARMV7.zip) |
...
| سیستم‌عامل (OS) | پردازنده (Architecture) | مناسب برای سیستم‌های... | لینک دانلود مستقیم |
| :--- | :--- | :--- | :--- |
| ویندوز (Windows) 🪟 | `AMD64` (64-bit) | ویندوز سرور، ویندوز ۱۰ و ۱۱ | [دانلود سرور ویندوز ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Windows_AMD64.zip) |
| ویندوز (Windows) 🪟 | `x86` (32-bit) | سیستم‌های قدیمی ۳۲ بیتی ویندوز | [دانلود سرور ویندوز x86 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Windows_X86.zip) |
| ویندوز (Windows) 🪟 | `ARM64` | دستگاه‌های ویندوزی مبتنی بر ARM | [دانلود سرور ویندوز ARM64 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Windows_ARM64.zip) |
| لینوکس (Linux) 🐧 | `AMD64` (64-bit) | سرورهای اوبونتو ۲۲.۰۴+، دبیان ۱۲+ | [دانلود سرور لینوکس (جدید) ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux_AMD64.zip) |
| لینوکس (Linux) 🐧 | `x86` (32-bit) | سیستم‌های قدیمی ۳۲ بیتی لینوکس | [دانلود سرور لینوکس x86 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux_X86.zip) |
| لینوکس (Legacy) 🐧 | `AMD64` (64-bit) | سرورهای قدیمی (اوبونتو ۲۰.۰۴، دبیان ۱۱) | [دانلود سرور لینوکس (سازگاری بالا) ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux-Legacy_AMD64.zip) |
| لینوکس (Legacy) 🐧 | `ARM64` | سیستم‌های ARM64 قدیمی‌تر که سازگاری بیشتری می‌خواهند | [دانلود سرور لینوکس Legacy ARM64 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux-Legacy_ARM64.zip) |
| لینوکس (ARM) 🐧 | `ARM64` | سرورهای ARM | [دانلود سرور لینوکس (ARM) ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux_ARM64.zip) |
| لینوکس (ARM) 🐧 | `ARMv7` | سرورهای ARM ۳۲ بیتی و دستگاه‌های embedded | [دانلود سرور لینوکس ARMv7 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux_ARMV7.zip) |
| لینوکس (ARM) 🐧 | `ARMv6` | بردهای ARM قدیمی‌تر و سیستم‌های سبک لینوکسی | [دانلود سرور لینوکس ARMv6 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux_ARMV6.zip) |
| لینوکس (ARM) 🐧 | `ARMv5` | دستگاه‌های ARM خیلی قدیمی و سیستم‌های embedded | [دانلود سرور لینوکس ARMv5 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux_ARMV5.zip) |
| لینوکس (Linux) 🐧 | `RISCV64` | بردها و سرورهای لینوکسی مبتنی بر RISC-V | [دانلود سرور لینوکس RISCV64 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux_RISCV64.zip) |
| لینوکس (MIPS) 🐧 | `MIPS` | سیستم‌ها و روترهای لینوکسی MIPS با endian بزرگ | [دانلود سرور لینوکس MIPS ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux_MIPS.zip) |
| لینوکس (MIPS) 🐧 | `MIPSLE` | سیستم‌ها و روترهای لینوکسی MIPS با endian کوچک | [دانلود سرور لینوکس MIPSLE ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux_MIPSLE.zip) |
| لینوکس (MIPS) 🐧 | `MIPS64` | سیستم‌های ۶۴ بیتی MIPS با endian بزرگ | [دانلود سرور لینوکس MIPS64 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux_MIPS64.zip) |
| لینوکس (MIPS) 🐧 | `MIPS64LE` | سیستم‌های ۶۴ بیتی MIPS با endian کوچک | [دانلود سرور لینوکس MIPS64LE ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Linux_MIPS64LE.zip) |
| مک‌اواس (macOS) 🍎 | `ARM64` | مک‌های جدید (سری M1 / M2 / M3) | [دانلود سرور مک (Apple Silicon) ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_MacOS_ARM64.zip) |
| مک‌اواس (macOS) 🍎 | `AMD64` | مک‌های اینتل | [دانلود سرور مک اینتل ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_MacOS_AMD64.zip) |
| ترموکس / اندروید 📱 | `ARM64` | محیط‌های جدید اندروید / Termux | [دانلود سرور ترموکس ARM64 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Termux_ARM64.zip) |
| ترموکس / اندروید 📱 | `ARMv7` | محیط‌های قدیمی‌تر اندروید / Termux ۳۲ بیتی | [دانلود سرور ترموکس ARMv7 ⬇️](https://github.com/nullroute1970/StormDNS/releases/latest/download/StormDNS_Server_Termux_ARMV7.zip) |
...
- سپس فایل `StormDNS_Client_Windows_AMD64.exe` را اجرا کنید. اگر همه چیز درست تنظیم شده باشد، کلاینت به سرور متصل می‌شود و آماده استفاده است.
...
```bash
sudo nano /etc/systemd/system/stormdns-client.service
```
...
```ini
[Unit]
Description=StormDNS Client Service
After=network.target
[Service]
Type=simple
ExecStart=/path/to/StormDNS_Client_Linux_AMD64 -config /path/to/client_config.toml
Restart=on-failure
[Install]
...
sudo systemctl enable stormdns-client
sudo systemctl start stormdns-client
```

همچنین برای دیدن لاگ‌ها می‌توانید از دستور زیر استفاده کنید:

```bash
sudo journalctl -u stormdns-client -f
```
...
- سپس فایل `StormDNS_Client_MacOS_ARM64` را اجرا کنید. اگر همه چیز درست تنظیم شده باشد، کلاینت به سرور متصل می‌شود و آماده استفاده است.
...
| پارامتر | مقدار نمونه | مقادیر مجاز / رفتار واقعی | توضیح کامل |
| :--- | :--- | :--- | :--- |
| `RESOLVER_BALANCING_STRATEGY` | `0` | فقط `0..4` | تعیین می‌کند کلاینت پکت‌هایش را روی resolverها چطور پخش کند.<br>`0` و `2` هر دو round-robin هستند.<br>`1` انتخاب تصادفی است.<br>`3` سعی می‌کند روی resolverهایی برود که loss کمتری داشته‌اند.<br>`4` بیشتر به resolverهای سریع‌تر گرایش دارد.<br>برای شرایط خیلی ناپایدار، معمولاً `3` یا `4` بهتر از `0` هستند. |
| `UPLOAD_PACKET_DUPLICATION_COUNT` | `2` | clamp به `1..8` | فقط برای پکت‌های **upload data** (`STREAM_DATA`، `STREAM_RESEND`).<br>چون حجم دیتای کاربر داخل اینهاست، هر duplication به‌طور مستقیم مصرف upload را چند برابر می‌کند.<br>روی لینک‌هایی با آپلود محدود (مثل شرایط معمول ایران) این مقدار را کم نگه دارید (`1`–`2`). |
| `DOWNLOAD_PACKET_DUPLICATION_COUNT` | `7` | clamp به `1..8` | فقط برای پکت‌های **download-support** (`STREAM_DATA_ACK`، `STREAM_DATA_NACK`).<br>اینها ACKهای کوچک ترافیک دانلود شما هستند.<br>duplicate کردنشان تقریباً هزینه‌ای روی آپلود ندارد ولی قابلیت اطمینان دانلود را زیاد می‌کند؛ روی شبکه‌های بد مقادیر `4`–`7` منطقی است. |
| `UPLOAD_SETUP_PACKET_DUPLICATION_COUNT` | `3` | clamp به `[UPLOAD_PACKET_DUPLICATION_COUNT, 8]` | برای پکت‌های **upload setup** (`STREAM_SYN`، `SOCKS5_SYN`).<br>حجمشان کم است ولی برای زمان ایجاد اتصال جدید حیاتی هستند. |
| `DOWNLOAD_SETUP_PACKET_DUPLICATION_COUNT` | `8` | clamp به `[DOWNLOAD_PACKET_DUPLICATION_COUNT, 8]` | برای پکت‌های **download setup / control** (`PACKED_CONTROL_BLOCKS`، `STREAM_CLOSE_READ`، `STREAM_CLOSE_WRITE`).<br>حجم کم ولی از دست رفتنشان teardown/control را مختل می‌کند؛ بالا نگه‌داشتن (`4`–`8`) معمولاً امن است. |
| `STREAM_RESOLVER_FAILOVER_RESEND_THRESHOLD` | `2` | clamp به `1..128` | اگر یک stream روی resolver ترجیحی خودش پشت سر هم resend بخورد، بعد از این آستانه کلاینت resolver ترجیحی آن stream را عوض می‌کند.<br>این مکانیزم باعث می‌شود یک stream روی resolver بد گیر نکند. |
| `STREAM_RESOLVER_FAILOVER_COOLDOWN` | `1.0` | clamp به `0.1..120` ثانیه | حداقل فاصله بین دو بار failover برای یک stream است.<br>اگر خیلی کم باشد stream مدام resolver عوض می‌کند و سیستم ناپایدار می‌شود.<br>اگر خیلی زیاد باشد stream دیرتر از resolver بد جدا می‌شود. |
...
| `RECHECK_INACTIVE_INTERVAL_SECONDS` | `60.0` | clamp به `60..86400` ثانیه | فاصله کلی دورهای recheck است.<br>یعنی health loop هر چند وقت یک‌بار به سراغ resolverهای غیرفعال برود. |
| `RECHECK_SERVER_INTERVAL_SECONDS` | `2.0` | clamp به `1..600` ثانیه | مبنای فاصله retry برای هر resolver غیرفعال است.<br>وقتی recheck fail می‌شود، کد با backoff و jitter از این مقدار استفاده می‌کند. |
| `RECHECK_BATCH_SIZE` | `30` | clamp به `1..1024` | در هر دور، چند resolver غیرفعال هم‌زمان/در یک batch بررسی شوند.<br>عدد خیلی بالا startup یا health loop را تهاجمی‌تر می‌کند. |
...
| `AUTO_DISABLE_TIMEOUT_WINDOW_SECONDS` | `90.0` | clamp به `1..86400` ثانیه | پنجره‌ای که در آن history timeout-only بررسی می‌شود.<br>اگر در این بازه فقط timeout ببینیم و شرط observation هم برقرار باشد، resolver غیرفعال می‌شود. |
| `AUTO_DISABLE_MIN_OBSERVATIONS` | `3` | clamp به `1..10000` | حداقل چند نمونه timeout لازم است تا auto-disable تصمیم بگیرد.<br>برای جلوگیری از false positive، این عدد نباید خیلی کم باشد. |
| `AUTO_DISABLE_CHECK_INTERVAL_SECONDS` | `4.0` | clamp به `0.25..600` ثانیه | هر چند وقت یک‌بار health loop وضعیت timeout-only resolverها را بررسی کند. |
...
> **نکته — لینک‌های با آپلود محدود (مثل ایران):** چون مصرف آپلود متناسب با `UPLOAD_PACKET_DUPLICATION_COUNT` زیاد می‌شود ولی duplication پکت‌های ACK عملاً رایگان است، یک پروفایل خوب این است:
> ```toml
> UPLOAD_PACKET_DUPLICATION_COUNT         = 1   # یا 2
...
> ```
> این تنظیمات مصرف آپلود را پایین نگه می‌دارد و در عین حال پایداری دانلود و ایجاد اتصال را به‌طور پررنگ محافظت می‌کند.
...
| پارامتر | مقدار نمونه | مقادیر مجاز / رفتار واقعی | توضیح کامل |
| :--- | :--- | :--- | :--- |
| `MIN_UPLOAD_MTU` | `40` | نباید منفی باشد | حداقل MTU قابل قبول برای مسیر upload است.<br>اگر resolver فقط MTU کمتر از این را جواب بدهد، برای upload کنار گذاشته می‌شود.<br>عدد پایین‌تر پایداری را بیشتر می‌کند ولی سرعت را کم می‌کند. |
...
| `MTU_TEST_RETRIES_RESOLVERS` | `3` | اگر کمتر از `1` باشد، fallback اعمال می‌شود | تعداد retry هر probe MTU وقتی `STARTUP_MODE = "resolvers"` (اسکن کامل از روی `client_resolvers.txt`).<br>روی resolverهای پر loss عدد بالاتر می‌تواند مفید باشد، ولی startup را کندتر می‌کند. |
| `MTU_TEST_TIMEOUT_RESOLVERS` | `2.0` | اگر `<=0` باشد، fallback اعمال می‌شود | timeout هر probe در تست MTU وقتی `STARTUP_MODE = "resolvers"`.<br>اگر شبکه شما دیرپاسخ است، این مقدار خیلی کم باعث حذف resolverهای سالم می‌شود. |
| `MTU_TEST_PARALLELISM_RESOLVERS` | `100` | اگر کمتر از `1` باشد، fallback اعمال می‌شود | تعداد تست‌های هم‌زمان MTU وقتی `STARTUP_MODE = "resolvers"`.<br>عدد بالا startup را سریع‌تر می‌کند ولی CPU و ترافیک بیشتری می‌سازد. |
| `MTU_TEST_RETRIES_LOGS` | `5` | اگر کمتر از `1` باشد، fallback اعمال می‌شود | تعداد retry هر probe MTU وقتی `STARTUP_MODE = "logs"` (راه‌اندازی سریع از روی resolverهای کش‌شده در logها). |
| `MTU_TEST_TIMEOUT_LOGS` | `2.0` | اگر `<=0` باشد، fallback اعمال می‌شود | timeout هر probe در تست MTU وقتی `STARTUP_MODE = "logs"`. |
| `MTU_TEST_PARALLELISM_LOGS` | `32` | اگر کمتر از `1` باشد، fallback اعمال می‌شود | تعداد تست‌های هم‌زمان MTU وقتی `STARTUP_MODE = "logs"`. |

### ۳.۴.۷) بخش ⚙️ Workerها، queueها و زمان‌بندی runtime

| پارامتر | مقدار نمونه | مقادیر مجاز / رفتار واقعی | توضیح کامل |
| :--- | :--- | :--- | :--- |
| `RX_TX_WORKERS` | `2` | clamp به `1..64` | تعداد workerهای مشترک runtime برای هر دو مسیر خواندن پاسخ‌های DNS و نوشتن queryهای خروجی.<br>این مقدار به صورت یکسان برای RX و TX استفاده می‌شود تا تنظیمات ساده‌تر بماند. |
| `TUNNEL_PROCESS_WORKERS` | `2` | clamp به `1..64` | تعداد workerهایی که packetهای دریافتی را decode و process می‌کنند.<br>اگر resolverهای زیادی دارید، این بخش روی CPU اثر زیادی دارد. |
...
| `TX_CHANNEL_SIZE` | `2048` | clamp به `64..65536` | ظرفیت channel ارسال packetها در runtime async.<br>اگر خیلی کوچک باشد زیر load سریع‌تر block می‌شود؛ اگر خیلی بزرگ باشد RAM بیشتری مصرف می‌کند. |
| `RX_CHANNEL_SIZE` | `2048` | clamp به `64..65536` | همان مفهوم برای مسیر دریافت. |
| `RESOLVER_UDP_CONNECTION_POOL_SIZE` | `128` | clamp به `1..1024` | برای هر resolver چند UDP connection قابل reuse نگه داشته شود.<br>عدد بزرگ‌تر هزینه ساخت connection را کم می‌کند ولی منابع بیشتری می‌گیرد. |
| `STREAM_QUEUE_INITIAL_CAPACITY` | `256` | clamp به `8..65536` | ظرفیت اولیه queue هر stream برای packetهای منتظر ارسال/مدیریت است.<br>برای streamهای زیاد، عدد مناسب باعث realloc کمتر می‌شود. |
| `ORPHAN_QUEUE_INITIAL_CAPACITY` | `64` | clamp به `4..4096` | ظرفیت اولیه queue packetهای orphan است؛ یعنی packetهایی که فعلاً هنوز stream مقصدشان attach نشده یا باید موقتاً جدا نگه داشته شوند. |
| `DNS_RESPONSE_FRAGMENT_STORE_CAPACITY` | `512` | clamp به `16..16384` | ظرفیت نگهداری fragmentهای پاسخ DNS پیش از مونتاژ کامل. |
...
| پارامتر | مقدار نمونه | مقادیر مجاز / رفتار واقعی | توضیح کامل |
| :--- | :--- | :--- | :--- |
| `PING_AGGRESSIVE_INTERVAL_SECONDS` | `0.200` | clamp به `0.05..30` ثانیه | فاصله ping در حالتی که ارتباط تازه داغ است و می‌خواهیم RTT و liveness را با حساسیت بالا نگه داریم. |
...
| `PING_WARM_THRESHOLD_SECONDS` | `5.0` | clamp به `0.1..600` ثانیه | آستانه‌ای که runtime براساس آن تصمیم می‌گیرد هنوز warm هستیم یا نه. |
| `PING_COOL_THRESHOLD_SECONDS` | `15.0` | clamp به `[PING_WARM_THRESHOLD_SECONDS, 1800]` | مرز ورود به فاز cool. |
| `PING_COLD_THRESHOLD_SECONDS` | `30.0` | clamp به `[PING_COOL_THRESHOLD_SECONDS, 3600]` | مرز ورود به فاز cold و ping بسیار کم‌فاصله. |
| `PING_WATCHDOG_TIMEOUT_SECONDS` | `300.0` | clamp به `10..3600` ثانیه، `0` غیرفعال می‌کند | اگر کلاینت traffic غیر-ping ارسال کرده باشد ولی در این مدت هیچ پاسخی از سرور دریافت نشود، session به‌اجبار ریست می‌شود. تایمر این watchdog تنها پس از اینکه همه resolverها اسکن شدند و session کاملاً عملیاتی شد شروع به کار می‌کند. این حالت zombie tunnel را که در آن همه resolverها بسته‌ها را بی‌صدا دور می‌اندازند و هیچ پیام ریست سمت سرور نمی‌رسد، بازیابی می‌کند. |

### ۳.۴.۹) بخش 🔄 ARQ، NACK و packing

| پارامتر | مقدار نمونه | مقادیر مجاز / رفتار واقعی | توضیح کامل |
| :--- | :--- | :--- | :--- |
| `MAX_PACKETS_PER_BATCH` | `5` | clamp به `1..64` | حداکثر تعداد control blockهایی که کلاینت در یک batch کنار هم می‌گذارد.<br>برای کاهش سربار ACK و control مفید است. |
...
| `ARQ_MAX_CONTROL_RETRIES` | `300` | clamp به `5..5000` | چند بار packetهای control اجازه retry داشته باشند. |
...
| `ARQ_DATA_NACK_MAX_GAP` | `64` | clamp به `0..32` در کد فعلی | تعیین می‌کند اگر جلوتر از `rcvNxt` packet رسید، تا چه فاصله‌ای gap را با NACK اعلام کنیم.<br>`0` یعنی NACK data عملاً خاموش است.<br>نکته مهم: sample مقدار `64` دارد، ولی کد فعلی آن را حداکثر تا `32` clamp می‌کند. |
...
| `UDP_READERS` | `2` | اگر `<=0` باشد auto-default | تعداد goroutineهای خواندن مستقیم از socket UDP.<br>عدد بالاتر در سرورهای پر ترافیک مفید است، ولی از یک حد به بعد فقط context switching را زیاد می‌کند. |
| `DNS_REQUEST_WORKERS` | `4` | اگر `<=0` باشد auto-default | تعداد workerهایی که requestهای ورودی را از front-door queue برمی‌دارند و به لایه session/decode می‌دهند. |
| `MAX_CONCURRENT_REQUESTS` | `4096` | اگر `<=0` باشد fallback | ظرفیت صف requestهای ورودی است.<br>اگر این صف پر شود، پکت‌ها drop می‌شوند و سرور rate-limited overload log می‌دهد. |
...
| پارامتر | مقدار نمونه | مقادیر مجاز / رفتار واقعی | توضیح کامل |
| :--- | :--- | :--- | :--- |
| `DEFERRED_SESSION_WORKERS` | `2` | clamp تا حداکثر `128` | تعداد workerهای deferred session است.<br>این workerها کارهای ordering-sensitive و setup-heavy را برای sessionها انجام می‌دهند.<br>کم بودن بیش از حد آن می‌تواند setup streamها را کند کند؛ زیاد بودن بی‌مورد هم contention می‌سازد. |
| `DEFERRED_SESSION_QUEUE_LIMIT` | `2048` | clamp به `256..14336` | ظرفیت queue deferred session برای کارهای معوق است.<br>اگر queue پر شود، setup یا کارهای deferred جدید ممکن است reject شوند. |
| `SESSION_ORPHAN_QUEUE_INITIAL_CAPACITY` | `128` | clamp به `4..4096` | ظرفیت اولیه queue packetهای orphan در هر session. |
| `STREAM_QUEUE_INITIAL_CAPACITY` | `256` | clamp به `8..65536` | ظرفیت اولیه queue هر stream در سمت سرور. |
| `DNS_FRAGMENT_STORE_CAPACITY` | `512` | clamp به `16..16384` | ظرفیت نگهداری fragmentهای DNS query داخل تونل. |
| `SOCKS5_FRAGMENT_STORE_CAPACITY` | `1024` | clamp به `16..16384` | ظرفیت نگهداری fragmentهای setup مربوط به SOCKS5. |
...
| پارامتر | مقدار نمونه | مقادیر مجاز / رفتار واقعی | توضیح کامل |
| :--- | :--- | :--- | :--- |
| `MAX_PACKETS_PER_BATCH` | `10` | اگر `<1` باشد fallback | حداکثر تعداد control blockهایی که سرور در یک پاسخ packed می‌کند.<br>نکته مهم: اگر مقدار نامعتبر بدهید، fallback واقعی کد `20` است. |
...
| `ARQ_WINDOW_SIZE` | `600` | clamp به `1..16384` | اندازه پنجره ARQ هر stream در سرور. |
...
| `ARQ_MAX_DATA_RETRIES` | `1200` | clamp به `60..100000` | سقف retry packetهای data. |
| `ARQ_DATA_NACK_MAX_GAP` | در sample نیامده | clamp به `0..32` | اگر packetهای out-of-order برسند، تا چه فاصله‌ای gap با NACK گزارش شود.<br>این پارامتر در کد هست و در README قبلی جا افتاده بود؛ اگر لازم دارید می‌توانید به sample config هم اضافه‌اش کنید. |
| `ARQ_DATA_NACK_INITIAL_DELAY_SECONDS` | `0.4` | clamp به `0..30` ثانیه | تأخیر اولیه قبل از ارسال NACK برای پکت‌های داده گم‌شده. سرعت درخواست ارسال مجدد را کنترل می‌کند. |
| `ARQ_DATA_NACK_REPEAT_SECONDS` | در sample نیامده | clamp به `0.1..30` ثانیه | برای یک sequence گمشده، NACK تکراری با چه فاصله‌ای دوباره صادر شود.<br>این هم در کد فعال است و باید در مستندات باشد. |
...
- افزایش تعداد بررسی همزمان ریزالورها (این بخش را با `STARTUP_MODE = "resolvers"` و اسکن کامل اجرا می‌کنیم):
...
حالا برنامه را اجرا کنید و صبر کنین تست ها انجام شود، پس از پایان عملیات برنامه را ببندید. لیست ریزالورهای سالم به صورت خودکار در پوشه `logs/` در قالب فایل `resolver_cache_<timestamp>.log` ذخیره می‌شود. هر خط شامل آدرس IP، پورت، دامنه، و مقادیر MTU است. می‌توانید آی‌پی‌ها را از این فایل کپی کرده و در `client_resolvers.txt` خود قرار دهید.
...
اگر روی یک سرور، پنل یا سرویس دیگری دارید، می‌توانید outbound آن را به SOCKS5 لوکال کلاینت StormDNS بدهید و از آن به‌عنوان مسیر خروجی استفاده کنید.
...
در این بخش چند پروژه مرتبط با StormDNS را معرفی می‌کنیم که توسط افراد مختلف توسعه داده شده‌اند و می‌توانند برای استفاده یا الهام گرفتن مفید باشند، البته توجه داشته باشید که این پروژه‌ها مستقل از StormDNS هستند و ممکن است با آن متفاوت باشند یا ویژگی‌های خاص خود را داشته باشند.
...
- [کلاینت اندروید MDV HN Edition](https://github.com/Hidden-Node/StormDNS-AndroidClient)
> یک اپلیکیشن اندروید که بر مبنای StormDNS ساخته شده که امکان اتصال به سرورهای StormDNS را از طریق گوشی فراهم می‌کند. این پروژه توسط Hidden Node توسعه داده شده است و می‌توانید آن را در لینک بالا مشاهده کنید.

- [کلاینت اندروید StormDNS GG](https://github.com/RevocGG/StormDNS-AndroidGG)
> یک اپلیکیشن اندروید دیگر که بر مبنای StormDNS ساخته شده است. این پروژه توسط RevocGG توسعه داده شده است و می‌توانید آن را در لینک بالا مشاهده کنید.

- [ابزار ساخت کانفیگ فارسی برای StormDNS](https://github.com/datacoder-io/StormDNS-ConfigMaker) - [آدرس وب جهت ساخت کانفیگ](https://datacoder-io.github.io/StormDNS-ConfigMaker)
> این ابزار به شما کمک می‌کند تا کانفیگ‌های StormDNS را به زبان فارسی و با توضیحات کامل بسازید. این پروژه توسط datacoder-io توسعه داده شده است و می‌توانید آن را در لینک بالا مشاهده کنید.
...
پروژه **StormDNS** با ترکیب Session Multiplexing، Resolver Balancing، Packet Duplication، لایه‌ی ARQ اختصاصی و Deferred Session Runtime روی بستر UDP/DNS محدودیت‌های تونل‌های معمول را دور می‌زند.
...
توسعه‌داده‌شده با ❤️ توسط: [**nullroute1970**](https://github.com/nullroute1970)

---

## 🍴 فورک (Fork)

این پروژه فورکی از [**MasterDnsVPN**](https://github.com/masterking32/MasterDnsVPN) است — نسخه [v2026.04.05.191930-7757d2d](https://github.com/masterking32/MasterDnsVPN/releases/tag/v2026.04.05.191930-7757d2d) توسط [masterking32](https://github.com/masterking32).
